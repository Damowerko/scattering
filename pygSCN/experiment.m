...
%% Import data
fields = {'PressureSeaLevelMBar', 'TemperatureC', 'WindSpeedKPH'};
k_matrix = ones(3);
...
options.filter.operator = 'L_norm'; % A L L_norm
...
options.filter.phi_sigma = 0.85;
options.filter.log_scale = 0; % if not use set to x<=0
options.filter.exp_scale = 1;
...
for M = 1:Mmax
    for J = 1:Jmax
        ...
        [filters,h]= filter_graph(A, @wavelet_morlet, @gaussian, options);
        ...
        for i = 1:n_samples
            ...
            temp = {};
            for j = 1:n_fields
                temp{j} = transform(data{j}(:,i), filters, options);
                temp{j} = temp{j}/norm(temp{j});
            end
            temp = vertcat(temp{:});
            transformed(:,i) = temp / norm(temp);   
        end
