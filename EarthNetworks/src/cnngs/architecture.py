...
from cnngs import graphtools as gt
...
class base_model(object):
    ...
    def evaluate(self, data, labels, sess=None):
        ...
        string = 'accuracy: {:.2f} ({:d} / {:d}), f1 (binary): {:.2f}, loss: {:.2e}'.format(
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope('inputs'):
                self.ph_data = tf.placeholder(
                    tf.float32, (self.batch_size, N[0], N[1]), 'data')
...
class cnngs(base_model):
    ...
    def lsigf(self, x, l):
        ...
        Fout = self.F[l]
        T, N, Fin = x.get_shape()
        # T matrices of size N x Fin:
        # [x_{1}^{(t)},...,x_{Fin}^{(t)}], t = 1,\ldots,T
        # x_{f}^{(t)} feature f, for training sample t (graph signal)
        # [x_{f}^{(t)}]_{n} feature f, for training sample t at node n.
        T, N, Fin = int(T), int(N), int(Fin)
        # T: number of training samples
        # N: number of nodes (intrinsic dimension)
        # Fin: number of input features
        if not isinstance(S, list):
            S = gt.GSO_powers_selected(S, N, K)
        assert len(S) == K
        ...
        for it in range(len(S)):
            thisS = scipy.sparse.csr_matrix(S[it])
            thisS = thisS.tocoo()
            indices = np.column_stack((thisS.row, thisS.col))
            thisS = tf.SparseTensor(indices, thisS.data, thisS.shape)
            thisS = tf.sparse_reorder(thisS)
        ...
        x0 = tf.reshape(x0, [N, Fin * T])  # N x Fin*T
        # One matrix, N x Fin*T: each row corresponds to one node. The
        # first T values of each row correspond to the value of the first
        # feature  at each of the T training samples, the second T to the
        # second feature, and so on.
        # [x_{1}^{(1)} ... x_{1}^{(T)} x_{2}^{(1)} ... x_{2}^{(T)} ...
        #	... x_{Fin}^{(1)} ... x_{Fin}^{(T)}]
        IN = scipy.sparse.csr_matrix(scipy.sparse.eye(N))  # Identity matrix
        x1 = x0
        x = tf.expand_dims(x1, 0)  # 1 x N x Fin*T
        ...
        for k in range(1, K):
            ...
            x1 = tf.sparse_tensor_dense_matmul(S[k], x0)  # N x Fin*T
            # [S^k x_{1}^{(1)}... S^k x_{1}^{(T)} S^k x_{2}^{(1)} ... S^k x_{2}^{(T)} ...
            #	... S^k x_{Fin}^{(1)} ... S^k x_{Fin}^{(T)}]
            x = concat(x, x1)
        # This got me two matrices so far:
        # Matrix 1:
        # [x_{1}^{(1)}... x_{1}^{(T)} x_{2}^{(1)} ... x_{2}^{(T)} ...
        #	... x_{Fin}^{(1)} ... x_{Fin}^{(T)}]
        # Matrix 2:
        # [Sx_{1}^{(1)}... Sx_{1}^{(T)} Sx_{2}^{(1)} ... Sx_{2}^{(T)} ...
        #	... Sx_{Fin}^{(1)} ... Sx_{Fin}^{(T)}]
        x = tf.reshape(x, [K, N, Fin, T])  # K x N x Fin x T
        # K supersets of N sets of Fin x T matrices
        # Matrices are like
        # [ [S^{k-1}x_{1}^{(1)}]_{n}   ... [S^{k-1}x_{1}^{(T)}]_{n}
        #   [S^{k-1}x_{2}^{(1)}]_{n}   ... [S^{k-1}x_{2}^{(T)}]_{n}
        #	...
        #   [S^{k-1}x_{Fin}^{(1)}]_{n} ... [S^{k-1}x_{Fin}^{(T)}]_{n} ]
        # where k is the index of superset K and n is the index of set N.
        x = tf.transpose(x, perm=[3, 1, 2, 0])  # T x N x Fin x K
        # T supersets of N sets of Fin x K matrices
        # [ [x_{1}^{(t)}]_{n}   ... [S^{K-1}x_{1}^{(t)}]_{n}
        #   [x_{2}^{(t)}]_{n}   ... [S^{K-1}x_{2}^{(t)}]_{n}
        #	...
        #	[x_{Fin}^{(t)}]_{n} ... [S^{K-1}x_{Fin}^{(t)}]_{n} ]
        # for t indexing the training sample T and n indexing the node N.
        x = tf.reshape(x, [T * N, Fin * K])  # T*N x Fin*K
        # The matrix should be as follows
        # [ [ x_{1}^{(1)} ... S^{K-1}x_{1}^{(1)}
        #	  		x_{  2}^{(1)} ... S^{K-1}x_{2}^{(1)}   ...
        #			x_{Fin}^{(1)} ... S^{K-1}x_{Fin}^{(1)};
        #     x_{1}^{(2)} ... S^{K-1}x_{1}^{(2)}
        #	  		x_{  2}^{(2)} ... S^{K-1}x_{2}^{(2)}   ...
        #			x_{Fin}^{(2)} ... S^{K-1}x_{Fin}^{(2)};
        #     ...
        #	  x_{1}^{(T)} ... S^{K-1}x_{1}^{(T)}
        #	  		x_{  2}^{(T)} ... S^{K-1}x_{2}^{(T)}   ...
        #			x_{Fin}^{(T)} ... S^{K-1}x_{Fin}^{(T)} ]
        # Filter: Fin*Fout filters of order K, i.e. one filterbank 
        # per feature pair.
        H = self._weight_variable([Fin * K, Fout], regularization=False)
        # If h^{fg} = [ h_{0}^{fg}, ... , h_{K-1}^{fg} ] are the Fin*Fout
        # filter taps, f = 1,...,Fin, g = 1,...,Fout. Then,
        # H = [ h_{0}^{11}     ... h_{0}^{Fout1}
        #		h_{1}^{11}     ... h_{1}^{Fout1}
        #		...
        #		h_{K-1}^{11}   ... h_{K-1}^{Fout1}
        #		h_{0}^{12}     ... h_{0}^{Fout2}
        #		h_{1}^{12}     ... h_{1}^{Fout2}
        #		...
        #		h_{K-1}^{12}   ... h_{K-1}^{Fout2}
        #		...
        #		h_{0}^{1Fin}   ... h_{0}^{FoutFin}
        #		...
        #		h_{K-1}^{1Fin} ... h_{K-1}^{FoutFin} ]
        #   = [ h^{11}   ... h^{Fout1}
        #		h^{12}   ... h^{Fout1}
        #		...
        #		h^{1Fin} ... h^{FoutFin} ]
        x = tf.matmul(x, H)  # T*N x Fout
        # Denote as H^{fg} the graph filter with filter taps given by
        # h^{fg}, then, the output of the matmul is
        # [ sum_g H^{1g}x_{g}^{(1)} ... sum_g H^{Fout g}x_{g}^{(1)}
        #   sum_g H^{1g}x_{g}^{(2)} ... sum_g H^{Fout g}x_{g}^{(2)}
        #   ...
        #   sum_g H^{1g}x_{g}^{(T)} ... sum_g H^{Fout g}x_{g}^{(T)} ]
        # [ u_{1}^{(1)} u_{2}^{(1)} ... u_{Fout}^{(1)}
        #	u_{1}^{(2)} u_{2}^{(2)} ... u_{Fout}^{(2)}
        #	...
        #	u_{1}^{(T)} u_{2}^{(T)} ... u_{Fout}^{(T)} ]
        # (Recall that u_{f} is a graph signal of size N)
        # Each (group of N) row(s) are the output features for that training
        # sample t.
        return tf.reshape(x, [T, N, Fout])  # T x N x Fout

    # We end up outputting T matrices, where each matrix is
    # [u_{1}^{(t)} ... u_{Fout}^{(t)}]
    # This is correct, as well.

    def regconv(self, x, l):
        K = self.K[l]
        Fout = self.F[l]
        T, N, Fin = x.get_shape()
        T, N, Fin = int(T), int(N), int(Fin)
